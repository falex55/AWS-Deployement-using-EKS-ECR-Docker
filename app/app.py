...
app = Flask(__name__)
model = pickle.load(open("model/Car Price Predicton RF.pkl", 'rb'))
...
def predict():
    ...
    if request.method == 'POST':
        ...
        print(f'transmission: {transmission_Manual}')
        print(f'transmission_semiAuto: {transmission_Automatic}')
        ...
        if output < 0:
            ...
            print(f'sell car at {output}')
            return render_template('index.html', prediction_text=f"You Can Sell The Car at {output} Rs.")
    else:
        return render_template('index.html')
